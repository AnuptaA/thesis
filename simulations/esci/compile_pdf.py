#!/usr/bin/env python3
"""
Compile all ESCI analysis PNGs from a processed run into a single PDF.
"""
...
SECTIONS = [
    ...
    ('set1', 'Set 1 -- Baseline (K=99, N=20)', [
        'baseline_hit_rate.png',
        'baseline_distance_calcs.png',
    ]),
    ('set2', 'Set 2 -- Cache Size Scaling (K=99, N=20)', [
    ...
    ('set3', 'Set 3 -- K/N Ratios', [
        'kn_heatmap_combined_angular.png',
        'kn_ratio_matrix_angular.png',
    ]),
]
...
def _default_processed_dir() -> Path:
    ...
    base = Path(__file__).parent.parent.parent / 'simulations' / 'esci' / 'processed'
    stamped = sorted(
...
def add_header_page(pdf: PdfPages, title: str, run_name: str):
    """Render a section header page into the PDF."""
...
def compile(processed_dir: Path, out_path: Path):
    """Compile all PNGs in processed_dir into a PDF at out_path."""
    ...
    with PdfPages(str(out_path)) as pdf:
        ...
        ax.axis('off')
        ax.text(0.5, 0.62, 'ESCI Simulation Analysis', transform=ax.transAxes,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compile ESCI analysis PNGs into a PDF')
