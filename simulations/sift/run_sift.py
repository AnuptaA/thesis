...
RESEARCH_BENCHMARKS = [
    ...
    "sift_b50k_c1024k100_t512n20_s42",
    "sift_b50k_c1024k100_t512n20_s43",
    "sift_b50k_c1024k100_t512n20_s44",
    # Set 2: Cache size scaling (3 sizes x 3 seeds; 1024 shared with Set 1)
    "sift_b50k_c256k100_t512n20_s42",
    "sift_b50k_c256k100_t512n20_s43",
    "sift_b50k_c256k100_t512n20_s44",
    "sift_b50k_c512k100_t512n20_s42",
    "sift_b50k_c512k100_t512n20_s43",
    "sift_b50k_c512k100_t512n20_s44",
    "sift_b50k_c2048k100_t512n20_s42",
    "sift_b50k_c2048k100_t512n20_s43",
    "sift_b50k_c2048k100_t512n20_s44",
]
...
def load_cache_ground_truth(benchmark_dir: Path, K: int) -> dict:
    ...
    Ground truth is saved by prepare_benchmark.py as cache_gt_angular_K{K}.npz.

    Args:
        benchmark_dir: benchmark directory (Path)
        K: cache K (number of neighbors per entry)

    Returns:
        dict mapping query_id -> (indices, distances, gap)
    """
    gt_file = benchmark_dir / f"cache_gt_angular_K{K}.npz"
    if not gt_file.exists():
        raise FileNotFoundError(
            f"Cache ground truth not found: {gt_file}\n"
            f"Run prepare_benchmark.py first to compute ground truth."
        )
    ...
    logger.info(f"Loaded cache ground truth: {len(ground_truth)} entries from {gt_file.name}")
    return ground_truth
...
def populate_cache_from_precomputed(
    ...
    """
    Populate KVCache using precomputed ground truth results.

    Args:
        simulator: CacheSimulator instance
        cache_queries: cache query vectors
        cache_ground_truth: precomputed ground truth dict (query_id -> (indices, distances, gap))
        base_vectors: base vectors for index lookup
    """
...
def run_benchmark(
    ...
    output_dir: str = "simulations/sift/raw",
    debug: bool = False,
    benchmark_base_dir: str = "datasets/sift",
):
    """
    Run cache simulations on a single SIFT benchmark (angular metric only).
    No brute force, no correctness validation -- only hit rate, distance calcs, timing.

    Args:
        benchmark_name: benchmark directory name under datasets/sift/
    ...
    print("=" * 80)
    print(f"SIFT BENCHMARK SIMULATION: {benchmark_name}")
    print("=" * 80)

    base, cache_q, test_q, config = load_sift_benchmark(str(benchmark_dir))

    print(f"\nBenchmark configuration:")
    print(f"  Main memory: {config['num_base_vectors']:,} vectors")
    ...
    # load precomputed cache ground truth (computed during prepare)
    ...
    for algorithm in algorithms:
        ...
        # simulator with dummy MainMemory -- never accessed since no brute force
        simulator = CacheSimulator(
            base_vectors=base,
        ...
        populate_cache_from_precomputed(simulator, cache_q, cache_ground_truth, base)
        ...
        sim_result = SimulationResult(
            ...
            correct_results=0,      # not tracked for SIFT/ESCI
...
def main():
    parser = argparse.ArgumentParser(
        description="Run SIFT cache simulations (angular metric, lemma variants only)",
    ...
    group.add_argument(
        ...
        type=str,
        metavar='NAME',
        help='single benchmark directory name to run (under datasets/sift/)'
    )
    group.add_argument(
        '--all',
        action='store_true',
